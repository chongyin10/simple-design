...
function User({ children }) {
    ...
    useEffect(() => {
        if (GlobalCls?.userInfo) {
            setUserInfo(GlobalCls?.userInfo);
            const avatarUrl = `/0/api/v1/user/image/profile_photo/${GlobalCls?.userInfo?.userId}.jpg?temp=${Date.now()}`;
    ...
    return (
        <div>
            ...
            >
                <div className='Navigation-r-li' style={{ height: '100%' }}>
                    {children}
                    <div className='Navigation-r-title'>{GlobalCls?.userInfo?.username || userInfo?.username}</div>
                </div>
            </Dropdown>
            <NewTeamModal
                openTeam={openTeam}
