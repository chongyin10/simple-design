...
import { ProcessingWithTTime, locationToProjectListPage } from '../../utils/hook/methods'
import './notification.less'
let dropDownDoading = false
let size = 5
let flag = true
function Notification(props){
  const { notificationView, onOk, onCancel, isThereAnyUnread} = props;
  ...
  // 获取类型
  const getType = (key) => {
    const findResult =  categoryIdList().find(item=>item.value === String(key))
    return findResult ? findResult.label:""
  }
  // 获取状态
  const getStatus = (key) => {
    const findResult =  approvalStatusList.find(item=>item.value === key)
    return findResult ? findResult.label : key
  }
  ...
  function jump(id) {
    locationToProjectListPage({path: "customerFeedback", queryStringObj: {approveId: id}})
  }
          ...
          {auditInfo.map((item, index) => (
            <li key={index}>
              ...
              <div className='cont'  dangerouslySetInnerHTML = {{ __html: item.content }}>
                {/* {item.username}提交的{getType(item.categoryId)}申请 
                <a onClick={() => jump(item.id)}>#{item.id}-</a> {getStatus(item.status)} */}
                {/* {item.content} */}
              </div>
